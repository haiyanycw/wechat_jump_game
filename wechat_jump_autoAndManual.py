...
def find_piece_and_board(im):
    ...
    last_pixel = np.array(im_pixel[board_x, i])
    for i1 in range(i+1,i+5):
        last_pixel += np.array(im_pixel[board_x, i1])
    last_pixel=tuple(last_pixel/5)
    # 从上顶点往下 +274 的位置开始向上找颜色与上顶点一样的点，为下顶点
    # 该方法对所有纯色平面和部分非纯色平面有效，对高尔夫草坪面、木纹桌面、
    # 药瓶和非菱形的碟机（好像是）会判断错误
    highK=min(i+274,piece_y)
    k=highK
    for k in range(highK, i, -1):  # 274 取开局时最大的方块的上下顶点距离
